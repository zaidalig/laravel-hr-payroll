...
@section('content')
<div class="d-flex justify-content-between mb-4"><p class="text-muted mb-0">System accounts and roles.</p><a href="{{ route('users.create') }}" class="btn btn-primary rounded-pill"><i class="fa-solid fa-plus me-1"></i>Add User</a></div>
...
<div class="col-md-3"><select name="role" class="form-select form-select-compact"><option value="">All Roles</option>@foreach(['owner','hr','accountant','viewer'] as $r)<option value="{{ $r }}" @selected(request('role')===$r)>{{ ucfirst($r) }}</option>@endforeach</select></div>
