...
<div class="col-md-3"><select name="status" class="form-select"><option value="">All Statuses</option>@foreach(['pending','paid'] as $s)<option value="{{ $s }}" @selected(request('status')===$s)>{{ ucfirst($s) }}</option>@endforeach</select></div>
...
</tbody></table></div>@if($payrolls->hasPages())<div class="card-footer bg-white">{{ $payrolls->links() }}</div>@endif</div>
@endsection
